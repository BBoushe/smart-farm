...
/**
 * Enhanced equipment sharing page with full CRUD functionality
 * Includes filtering, search, and listing management
 */
export default function EquipmentPage() {
  ...
  const loadListings = async () => {
    setLoading(true)
    try {
      const data = await dataService.getListings({ category: "equipment" })
      setListings(data)
  ...
  const handleCreateListing = async (data: Partial<Listing>) => {
    try {
      await dataService.createListing({
        ...
        category: "equipment",
        status: "active",
        userId: "U001", // In real app, get from auth
        images: data.images || [],
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">Loading equipment listings...</div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold text-green-800 mb-2">Equipment Sharing</h1>
          <p className="text-stone-600">Share equipment with fellow farmers to reduce costs and improve efficiency</p>
        </div>
      ...
      <ListingFilters
        ...
        categories={["tractors", "harvesters", "planting", "tillage", "irrigation"]}
        types={["share", "rent"]}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredListings.map((listing) => (
          <ListingCard
            key={listing.id}
            listing={listing}
            showActions={listing.userId === "U001"} // Show actions for own listings
      ...
      {filteredListings.length === 0 && (
        <div className="text-center py-12">
          <p className="text-stone-600 text-lg">No equipment listings found matching your criteria.</p>
